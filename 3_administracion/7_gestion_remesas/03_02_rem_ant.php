...
mod_sql("
INSERT INTO cobros
(cod_empresa,cod_cobro,tabla,cod_cliente,nombre_cliente,fecha_cobro,hora_cobro,total_cobro,cod_cuenta,desc_cobro,cod_remesa)
VALUES
('$cod_empresa','$cod_cobro','$tabla','".$lin[$i]['cod']."','".addslashes($lin[$i]['nom'])."','".$lin[$i]['venc']."','".$hora_creacion."','".$lin[$i]['cant']."','$cod_cuenta','".addslashes($concept)."','".$emp['cod_remesa']."')");
...
SELECT *
FROM $tabla
WHERE cod_forma = 'REMESA' AND num_cuenta RLIKE '^[0-9]{4}-[0-9]{4}-[0-9]{2}-[0-9]{10}$' AND fecha_baja = 0
...
{
$cod_cliente=$c[$i][$campo1];
$nombre_cliente=$c[$i][$campo2];
...
$acum='430';
$codigo = $cod_cliente + 9000;

$cod_cuenta=sprintf("%02s".$acum."%04s","01",$codigo);


// Comprobamos si existe:
$d=sel_sql("SELECT saldo FROM cuentas WHERE cod_cuenta = '$cod_cuenta'");
//echo "SELECT saldo FROM cuentas WHERE cod_cuenta = '$cod_cuenta'";
...
$nombre_cliente .= ' '.$c[$i]['apellido1'].' '.$c[$i]['apellido2'];
}
...
<input type="text" name="rem[<? echo $i; ?>][cant]" id="rem[<? echo $i; ?>][cant]" maxlength="7" size="9" value="50"></td>
            <td>&nbsp;</td>
          </tr>
<?
} // Fin de for
