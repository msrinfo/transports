...
if (cod_factura_ini > cod_factura_fin || isNaN(cod_factura_ini) || isNaN(cod_factura_fin))
	{
	alert("Valores incorrectos.\nRecuerde: El código inicial no puede ser mayor que el código final.");
	}
...
mostrar(event,'3_2_fac_serv_impr.php','cod_empresa',cod_empresa,'cod_factura_ini',cod_factura_ini,'cod_factura_fin',cod_factura_fin,'logo',logo,'','','','','','','','','','','','');
}
...
pdf.src='/pdf/public_html/demo/index.php?url_fac=<? echo $servidor_web_js; ?>/<? echo $carpeta_js; ?>/3_administracion/5_gestion_facturacion/3_2_fac_serv_impr.php&carpeta=<? echo $carpeta_js; ?>&usuario_any=<? echo $usuario_any_js; ?>&cod_empresa='+cod_empresa+'&cod_factura_ini='+cod_factura_ini+'&cod_factura_fin='+cod_factura_fin+'&fac_accion='+fac_accion; //aqui no se cambia el 3_2_ver_fac.php NUNCA!!!
...
<form name="form1" method="post" action="" onSubmit="">
  <tr class="titulo"> 
    <td colspan="16">Factures de Serveis C&ograve;pies</td>
  </tr>
  <tr>
    <td width="3"></td>
    <td colspan="4"></td>
  ...
  <tr>
    ...
    <td>
<input name="cod_factura_fin" type="text" id="cod_factura_fin" size="6" maxlength="6" onBlur="buscar_conta(event,'facturas',cod_factura_fin.value,'cod_factura',cod_factura_fin.value,'cod_empresa',cod_empresa.value,'origen','S','','','','','','','fac_fin');">
  ...
  <tr>
    <td></td>
    <td colspan="5" align="center"><input name="imprimir" type="button" id="imprimir" value="Veure" onClick="errores(event)">
      <input name="imprimir_logo" type="button" id="imprimir_logo" value="Veure Logo" onClick="errores(event)">
	  <input name="gen_pdf" type="button" id="gen_pdf" onClick="facturas_copias_tt_serv(event)" value="Gen. PDF">&nbsp;
      <input name="enviar_mail" type="button" id="enviar_mail" onClick="facturas_copias_tt_serv(event)" value="Enviar E-Mail">&nbsp;
      <input name="pdf_mail" type="button" id="pdf_mail" onClick="facturas_copias_tt_serv(event)" value="Gen. PDF y Enviar E-Mail">&nbsp;	
