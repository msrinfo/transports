...
// Si no hemos recibido cod_albaran, establecemos condiciones y seleccionamos clientes:
if (!$cod_albaran)
{
// Establecemos el valor de IVA (se tiene en cuenta el iva de la ficha del cliente):
$num_iva="and albaranes.tipo_iva = '$tipo_iva'";
...
$periodo="and fecha_carga BETWEEN '$fecha_ini' AND '$fecha_fin'";
//echo "$periodo";

$clientes_alb="
SELECT DISTINCT albaranes.cod_cliente 
FROM albaranes, clientes
WHERE albaranes.cod_cliente = clientes.cod_cliente and cod_empresa = '$cod_empresa' and estado != 'f'
$tipo_factura $num_iva $cliente $periodo  
ORDER BY albaranes.cod_cliente, albaranes.cod_albaran";
}

else
{
$clientes_alb="SELECT cod_cliente FROM albaranes WHERE cod_albaran = '$cod_albaran' and cod_empresa = '$cod_empresa' and estado != 'f'";
}

//echo "<br /> $clientes_alb <br />";

$consulta_cli_alb=mysql_query($clientes_alb, $link) or die ("<br /> No se han seleccionado clientes de albaranes: ".mysql_error()."<br /> $clientes_alb <br />");

// Obtenemos el nº de albaranes
$num_albaranes=mysql_num_rows($consulta_cli_alb);
//echo "<br /> num_albaranes: $num_albaranes <br />";


// Si NO obtenemos albaranes, no continuamos:
if ($num_albaranes == 0)
{
?>
<script type="text/javascript">
alert('No se han encontrado albaranes pendientes de facturar.');
...
// Por cada cliente con albaranes pendientes, crearemos una factura con sus albaranes:
...
$nombre_cliente=addslashes($fila["nombre_cliente"]);
...
$fecha_carga=$fac_fecha;
}

else
{
$consulta="SELECT base as fac_bruto, nombre_cliente, tipo_iva, fecha_carga FROM albaranes WHERE cod_albaran = '$cod_albaran' and cod_empresa = '$cod_empresa' and estado != 'f'";
}
...
if ($cod_albaran)
{
$nombre_cliente=addslashes($resultado["nombre_cliente"]);
...
if (!$cod_albaran)
{
$actualizar_alb="UPDATE albaranes SET cod_factura = '$cod_factura', estado = 'f' WHERE cod_empresa = '$cod_empresa' and estado = ''
$tipo_factura $num_iva $cliente $periodo";
}

else
{
$actualizar_alb="UPDATE albaranes SET cod_factura = '$cod_factura', estado = 'f' WHERE cod_albaran = '$cod_albaran' and cod_empresa = '$cod_empresa' and estado = ''";
}

//echo "<br /> actualizar_alb: $actualizar_alb <br />";
$result_alb = mysql_query($actualizar_alb, $link) or die ("<br /> No se han actualizado albaranes: ".mysql_error()."<br /> $actualizar_alb <br />");
...
mostrar(sim_clic_impr(),'3_2_fac_alb_impr.php','cod_factura_ini','<? echo $cod_factura_ini; ?>','cod_factura_fin','<? echo $cod_factura_fin; ?>','cod_empresa','<? echo $cod_empresa; ?>','','','','','','','','','','','','','','');

// Si hemos facturado un albarán desde albaranes, cerramos ventana y recargamos el interfaz de albaranes:
if (opener)
{
window.close();
opener.location.href="/<? echo $carpeta; ?>/3_administracion/4_gestion_albaranes/1_1_albaranes.php?cod_albaran=<? echo $cod_albaran; ?>&cod_empresa=<? echo $cod_empresa; ?>";
}
...
if (!fac_fecha || (cod_albaran && (fecha_ini || fecha_fin)) || (!cod_albaran && (!tipo_iva || !fecha_ini || !fecha_fin)))
{
...
<form name="form1" method="post" action="">
          <tr class="titulo"> 
            <td colspan="4">Facturaci&oacute; d'Albarans</td>
          </tr>
          <tr> 
            <td width="7">&nbsp;</td>
            <td width="407">&nbsp;</td>
          ...
          <tr>
            ...
            <td><input name="cod_albaran" type="text" id="cod_albaran" size="6" maxlength="6" value="<? echo $cod_albaran; ?>" onBlur="buscar_conta(event,'albaranes',cod_albaran.value,'cod_albaran',cod_albaran.value,'cod_empresa',cod_empresa.value,'','','','','','','','','alb_fac')" <? echo "$clase_alb"; ?>>
                <img src="/comun/imgs/lupa.gif" onClick="abrir(event,'cod_albaran');">
          ...
          <tr>
            ...
            <td><input name="tipo_iva" type="text" id="tipo_iva" size="2" maxlength="2" value="<? if (!$cod_cliente && !$cod_albaran) {$tipo_iva=$val_iva[0];} echo $tipo_iva; ?>" <? echo "$clase_cli"; ?>></td>
            <td>&nbsp;</td>
          </tr>
          <tr>
            <td>&nbsp;</td>
...
if ($cod_albaran)
{
// Si cod_albaran no está facturado y lo hemos recibido desde albaranes, lo facturamos:
$estado=sel_campo("estado","","albaranes","cod_albaran = '$cod_albaran' and cod_empresa = '$cod_empresa'");

if ($estado=="f")
{
?>
<script type="text/javascript">
alert("Albarán <? echo $cod_albaran; ?> de la empresa <? echo $cod_empresa; ?> ya facturado.");
location.href=direccion_conta('');
</script>
<?
}
...
if ($direccion=="1_1_albaranes.php")
{
?>
<script type="text/javascript">
document.forms[0].submit();
